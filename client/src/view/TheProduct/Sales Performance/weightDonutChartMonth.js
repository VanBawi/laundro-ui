...
import { monthWeightDistribution } from '../../../reducer/salesData';
import { Skeleton } from 'antd';

const WeightDemoDonutMonth = ({ year, month, searchNow, salesPerformanceFilter, operatorId, mchType }) => {
	const dispatch = useDispatch();
	const { weightMonth } = useSelector((state) => state.sales);
	...
	useEffect(() => {
		...
		if (searchNow || salesPerformanceFilter) {
			dispatch(monthWeightDistribution({ year, month, operatorId }));
		}
	}, [dispatch, year, month, searchNow, salesPerformanceFilter, operatorId]);

	useEffect(() => {
		if (weightMonth) {
			const d = weightMonth.length
				? weightMonth
						.filter((x) => x.machine_type === mchType)
						.map((e) => {
							// console.log(e.machine_type)
							return {
								...e,
	...
	// console.log('weightMonth chart 4', weightMonth);
...
export default WeightDemoDonutMonth;
