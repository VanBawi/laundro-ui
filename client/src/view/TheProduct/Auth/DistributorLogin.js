...
import { loginDistributor } from '../../../reducer/userReducer';
import { useNavigate, Link } from 'react-router-dom';

const DistributorLogin = () => {
	const dispatch = useDispatch();
	const navigate = useNavigate();
	const { distributor, error } = useSelector((state) => state.user);
	...
	const onFinish = (values) => {
		...
		dispatch(loginDistributor(values));
	};

	useEffect(() => {
		if (distributor && submitted) {
			setSubmitted(false);
			if (distributor) {
				if (distributor.role === 'distributor' && distributor.permissions && distributor.permissions.length) {
					const allPages = [
					...
					const availablePermissions = distributor.permissions
					...
					navigate(firstPage.pagePath, { state: { userType: 'distributor' } });
				} else navigate('/mainDashboard', { state: { userType: 'distributor' } });
			}
		}
	...
	return (
		<div className='login-main-container d-sm-flex justify-content-evenly'>
			...
			<div className='card mx-3 mt-3 py-5 px-4' style={{ maxWidth: '500px' }}>
				<div>
					<h2 className='pb-2'>Franchisor Login</h2>
					<p className='pb-2'>Please enter your user name and password to login</p>
				</div>
				<Form name='Franchisor_login' onFinish={onFinish}>
					<Form.Item
						name='username'
						rules={[
							{
